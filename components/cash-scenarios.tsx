    ...
    <div className="mt-6 grid gap-5 md:grid-cols-3">
      {scenarios.map((scenario) => {
        ...
        return (
          <article
            key={scenario.name}
            className="flex min-h-[240px] flex-col gap-5 rounded-3xl border border-white/60 bg-white/85 p-6 shadow-inner backdrop-blur transition hover:-translate-y-0.5 hover:shadow-xl dark:border-slate-800/70 dark:bg-slate-900/70"
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              ...
              <span className="inline-flex shrink-0 items-center rounded-full bg-primary-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-primary-600">
                {scenario.runway}
              </span>
            </div>
            <p className="text-sm leading-relaxed text-slate-500 dark:text-slate-400 sm:leading-6">
              {scenario.narrative}
            </p>
            <div className="flex flex-col gap-1.5 text-xs uppercase tracking-wide text-slate-400">
              ...
              <span className="font-medium text-slate-500 dark:text-slate-300 leading-relaxed">
                {scenario.risk}
              </span>
            </div>
          </article>
